...
const DealCard = ({ deal, onClick, onDelete, onOpenNotes, onOpenReminder }: DealCardProps) => {
  ...
  return (
    <article
      className={`group rounded-2xl border border-slate-200/80 bg-white/90 p-5 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:border-slate-300 hover:shadow-lg motion-reduce:transition-none animate-[fade-in_180ms_ease-out] ${
    ...
    >
      ...
      <div className="mt-4 flex items-center justify-between text-xs text-slate-500">
        ...
        <div className="flex items-center gap-2">
          ...
          {overdue && (
            <span className="inline-flex items-center gap-1 rounded-full border border-rose-200 bg-rose-50 px-2 py-0.5 text-[11px] font-semibold text-rose-700 shadow-sm">
              <svg
                className="h-3 w-3"
                viewBox="0 0 20 20"
                fill="currentColor"
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        {onDelete ? (
          <button
            ...
            onPointerDown={(event) => event.stopPropagation()}
            className="rounded-full border border-rose-200 px-3 py-1 text-xs font-semibold text-rose-700 transition hover:border-rose-300 hover:bg-rose-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-200"
          >
            Supprimer
          </button>
        ) : null}
